...
public class Window extends JFrame{
	...
	private int block_size;
  	...
  	// Fenêtre de base
  		block_size = 27 - ((maze.getMaze().length > maze.getMaze()[0].length) ? ((maze.getMaze().length-1)/2)/5 : ((maze.getMaze()[0].length-1)/2)/5);
	    	...
	    	if(i > 0 ){
	    		...
	    		//System.out.println("diff_x: "+diff_x+" diff_y: "+diff_y);
	      		for(int j = 0; j < 2*block_size;j++){
	      			x= maze.get_completeWay().get(i-1).get(1)*2*block_size+(25+block_size*4/5)+(diff_x*j/(2*block_size));// de base 45
	      			y= maze.get_completeWay().get(i-1).get(0)*2*block_size+(25+block_size*4/5)+(diff_y*j/(2*block_size));
